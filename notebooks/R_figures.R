...
p <- read_csv("../data/processed/normalized_errors.csv") %>%
  select(-X1) %>%
  filter(author != "Ishii" & author != "Ec_core") %>%
  mutate(author = factor(author)) %>%
  mutate(author = fct_relevel(author, levels=c("Khodayari", "Kurata", "Millard", "iML1515", "ECC2","Exp_ECC2", "Exp_iML1515"))) %>%
  ggplot(aes(x=author, y=normalized_error, color=author, label=sample_id)) 

errors_p <- p + geom_point() + geom_jitter(width=0.3) + ggtitle("Normalized error knockout simulations")
